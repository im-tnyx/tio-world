...
2. **Library** — one canonical Workout-owned route reached from a Workout Home entry, with capability-gated sections for Programs, Routines, Plans / Training Plans and Exercises. It is not a guided default tab or a Workout-local content tab; a future configurable navigation may promote the same route. See [Library](library.md).
...
4. **Exercises** — a dedicated Exercises screen (Library → Exercises) for catalog, search, filters and detail, backed first by a local, versioned JSON catalog; the same capability also provides the exercise picker inside Routine/Program builders. It is not a direct workout-start surface. See [Exercises and Exercise Picker](exercise-search.md).
