...
- Whether that trailing Calories + Protein section-header group renders at all is presentation-controlled by one preference, `showMealSectionNutrition` (default ON). It is one switch for the whole group, never a separate Calories/Protein choice; turning it off removes the entire group so the divider/title reclaim the freed width, and it never hides calories/protein inside an individual card, never fabricates a missing value, and never touches aggregate calculation.
...
- N14 Meal Diary display preferences are live on cards and section headers. `showMealTimes` hides only the visible time label and never chronology; `mealNotesEnabled` hides note presentation without touching stored notes; `showMealNotePreview` can add at most one ellipsized note line; `showMealSectionNutrition` controls the section-header nutrition group described above. The default remains time ON, Meal Notes ON, preview OFF, section nutrition ON.
...
- A blank optional Quick Add value means absent, not zero. Explicit `0` remains a known zero. Manual/coarse Quick Add and Quick Edit enforce one shared V1 amount policy: Calories must be `0..10000 kcal`; Carbs, Protein and Fat must each be `0..1000 g`; present values allow at most one decimal place. Excess precision is rejected rather than silently rounded. Negative, unparseable, non-finite, above-maximum and unsupported scientific-notation input keeps the text the reader typed and gets a message on its own line beneath the row, never colour alone. Existing out-of-policy manual rows remain readable in Quick Edit but `Save Changes` stays blocked until the visible invalid amount is corrected; accepted ordinary floating-point representation noise is canonicalized only for editor hydration so a valid stored value remains editable.
