...
- the built-in Exercise catalog is bundled/versioned application content owned by the Workout capability; since W3A2a it ships as `apps/features/workout/assets/exercises/exercise_catalog.json`, registered in `tio_feature_workout`, loaded by `AssetBundleExerciseCatalogSource` through a versioned document envelope and validated/mapped by the W3A1 parser (see [Exercise search](screens/exercise-search.md#first-slice-data-source)); the Exercises screen and route remain pending W3A2b;
...
- `apps/core` owns the reusable inline date calendar (`TioDateCalendar`): compact date strip, expandable inline month grid, generic per-date decoration, gesture and accessibility mechanics. Consuming features own their selected date, their `minDate`/`maxDate` policy and what a decoration means; `MealPlan`, `PlannedMeal`, `TrainingPlan`, `PlannedWorkout`, calories and targets must never enter its API. Nutrition owns the Meal Diary date state and its `maxDate = localToday` rule. First day of week is a single app-global Calendar Preferences value consumed by every calendar surface; no feature may own a `nutritionFirstDayOfWeek`, `workoutFirstDayOfWeek` or equivalent. The future Planning Calendar and Progress Calendar are separate surfaces, not modes of this component.
- `apps/core` owns the generic anchored DateTime popup (`TioDateTimePickerPopup`) and controlled Cupertino Date + Hour + Minute + AM/PM wrapper (`TioDateTimeWheelPicker`), including generic bounds and caller-supplied resolution hooks. Features own anchors, default values, clock freshness, domain validation, draft lifetime, persistence, and submit behavior. Nutrition's Quick Add keeps consumed time as route-local draft state and supplies its no-future resolver; no meal semantics belong in Core.
