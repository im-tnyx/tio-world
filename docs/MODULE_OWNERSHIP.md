...
- S0-B2 explicitly assigns `HydrationPreferences`, its repository contract and Supabase adapter to Settings. `apps/app` only constructs/injects them. `public.user_hydration_preferences` is the dedicated account-synced storage target; no second owner belongs in Progress, shared, or a speculative hydration package. Future hydration logging must consume this preference. This narrow exception does not move `WellnessTargetsData`, Water Goal, Units, Profile or other feature ownership into Settings.
