...
- `apps/core` owns generic wheel mechanics (`TioWheelPickerColumn`/`TioWheelPickerFrame`) and the controlled Date + Hour + Minute + AM/PM composition (`TioDateTimeWheelPicker`), including coherent calendar rollover and caller-supplied bounds/resolution hooks. Features own default values, clock freshness, domain validation, draft lifetime, persistence, and submit behavior. Nutrition's Quick Add keeps consumed time as route-local draft state and supplies its no-future resolver; no meal semantics belong in Core.
