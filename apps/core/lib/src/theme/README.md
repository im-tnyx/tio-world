...
`TioNavigationTokens.topBarTitleGap` is the canonical visible gap between the AppBar leading icon and the title, and aliases `TioSpacing.sm` (8dp). Flutter centres the 24dp leading icon in a 56dp slot, 16dp from the slot edge, so `TioNavigationTokens.topBarTitleSpacing` derives the matching (negative, -8dp) `titleSpacing` from the gap, and `TioTheme` installs it through `ThemeData.appBarTheme.titleSpacing`. With a back or close button the title starts at 48dp; a later change to the gap changes only `topBarTitleGap`. Standard AppBars must not set a per-screen `titleSpacing`.

The negative spacing also widens the title slot by the same amount on the trailing side. A text title reaches that far only when it is long enough to ellipsize, and then still ends inside the action's own 12dp icon padding. A title that fills its slot (for example the Exercises search field) must pad its end by `-TioNavigationTokens.topBarTitleSpacing` so it stays out of the trailing action's slot. Every standard AppBar carries a leading widget; an AppBar without one would put its title past the screen edge, so a bar without a leading widget uses `TioShellStatusTopBar` (which keeps its own `TioSpacing.lg` title inset) or passes an explicit governed `titleSpacing`.
