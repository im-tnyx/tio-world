...
`TioDateCalendar` is the reusable inline date calendar: a compact horizontal date strip and an expandable inline month grid that are two renderings of one caller-controlled `selectedDate`. It has no component-token file, because the component-token admission gate above is not met: it consumes `TioSize`, `TioSpacing`, `TioRadius`, `TioStroke`, `TioOpacity`, `TioFontWeight`, `context.tioColors` and `context.tioMotion` directly. Callers own `selectedDate`, `localToday`, `minDate`/`maxDate` and the resolved first day of week; per-date visuals arrive as a generic `TioDateDecoration` (progress, generic fill, marker count) so Core renders presentation values without learning any feature's domain. `onVisibleDateRangeChanged` reports the inclusive seven-day week or calendar month owned by the active pager page, allowing a caller to distinguish selection from viewport without putting feature policy in Core. At normal text scale each date uses a 30dp governed cell. A selected, enabled date is a tonal disk (`colors.primary` at `TioOpacity.opacity12`) filling out to radius 13, the progress ring's inner edge, so progress stays the outer visual boundary with no decorative gap and the disk never resizes with progress; numeral colours are unchanged by selection. When a caller supplies a `TioDateFill`, that fill stays authoritative on a selected date and selection falls back to the thin 0.5dp `colors.primary` ring inside progress. Disabled and neighbouring-month cells draw no selection. The progress arc resolves `colors.progress`, while generic fill resolves `colors.primary`. Known limitation: a plain selected date and an unselected `TioDateFill.soft` date with progress render identically, and selected + `solid` + progress matches unselected `solid` + progress; resolve this before the first `TioDateFill` consumer. `progress: null` means unavailable and renders no progress ring, while `progress: 0` remains a known-zero value and is deliberately distinct. Callers own the semantic/domain meaning and must supply normalized presentation data; do not add domain parameters to the calendar or turn it into a Planning or Progress calendar. `tioOrderedWeekdayLabels` and `tioWeekdayName` are exported beside it. The calendar's own weekday header draws its columns from the first; the second names a single day and is what the Settings first-day-of-week choice labels its options with. Both format from the locale; never hard-code an English weekday string beside them.
...
`TioAppBar` is the standard phone top bar for pushed screens; feature and app screens use it instead of a raw `AppBar`. `TioNavigationTokens.topBarTitleGap` is the canonical visible gap between its leading back/close icon and the title, and aliases `TioSpacing.sm` (8dp); a later change to the gap changes only this token. Flutter centres the 24dp leading icon in a 56dp slot, 16dp from the slot edge, so `TioNavigationTokens.topBarTitleSpacing` derives the matching (negative, -8dp) `titleSpacing` from the gap. `TioAppBar` applies it only when the bar has a leading widget (explicit, or implied by a poppable route or a drawer); without one the title keeps the `TioSpacing.lg` edge inset. The negative spacing lays the title's start over the end of the leading slot, so `TioAppBar` keeps that strip of the title clear, for pointers and for accessibility bounds (mirrored in right-to-left layouts). The leading button keeps its whole tap target and its accessibility bounds, and an interactive title such as a search field is still hit exactly where it is painted. The title keeps AppBar's header semantics (`header`, and `namesRoute` where AppBar sets it). Because `AppBar` applies `titleSpacing` on both sides, `TioAppBar` pads the title end so it always stays `TioSpacing.lg` clear of the actions or the trailing edge. The title is start-aligned on every platform (`centerTitle: false`), so iOS matches Android.

Standard screens must not set a per-screen `titleSpacing`, and `TioTheme` does not install one: a theme-wide negative spacing would push leading-less titles past the screen edge. The shell top bars keep their own contracts: `TioShellStatusTopBar` sets its own `TioSpacing.lg` title inset, and `TioShellTopBar` has no title.
