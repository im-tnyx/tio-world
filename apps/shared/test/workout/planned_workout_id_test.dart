  ...
  group('PlannedWorkoutId', () {
    ...
    test('rejects blank, whitespace-wrapped and malformed values', () {
      for (final value in [
        ...
        expect(() => PlannedWorkoutId(value), throwsArgumentError,
            reason: value);
      }
    });

    test('equal for the same canonical value with a consistent hash', () {
      final a = PlannedWorkoutId(uuidV4);
      final b = PlannedWorkoutId(uuidV4.toUpperCase());
    ...
    test('unequal for different values', () {
      expect(PlannedWorkoutId(uuidV4), isNot(PlannedWorkoutId(uuidV7)));
    });

    test('is not equal to another Workout identity with the same UUID', () {
      final Object id = PlannedWorkoutId(uuidV4);
