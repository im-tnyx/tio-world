  ...
  group('OnboardingDraftSnapshotDtoMapper', () {
    ...
    test('serializes and deserializes canonical draft losslessly', () {
      ...
      final snapshot = OnboardingDraftSnapshot(
        schemaVersion: 1,
        draft: draft,
        updatedAt: DateTime.utc(2026, 8, 14, 12, 0, 0),
      );

      final json = mapper.toJson(snapshot);

      // Verify no enum ordinals (stable string IDs)
      ...
      expect(deserialized.schemaVersion, equals(1));
      expect(deserialized.draft.selectedMode, equals(AppMode.hybrid));
      expect(deserialized.draft.workoutIntroChoice, equals(WorkoutIntroChoice.setupNow));
      expect(deserialized.draft.currentStepId, equals(OnboardingStepId.profileBasics));
      expect(deserialized.draft.completedStepIds, contains(OnboardingStepId.mode));
      ...
      expect(deserialized.draft.profile.goals, contains(ProfileGoal.buildMuscle));
      ...
      expect(deserialized.draft.workout.equipment, contains(WorkoutEquipment.dumbbells));
      expect(deserialized.draft.workout.experienceLevel, equals(WorkoutExperienceLevel.intermediate));
      expect(deserialized.draft.workout.trainingDays, contains(WorkoutTrainingDay.monday));
