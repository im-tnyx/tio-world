...
/// Resends a passwordless Phone OTP by repeating Supabase `signInWithOtp`.
final class ResendPhoneOtpUseCase {
  ...
  Future<PhoneOtpRequestResult> call({
    ...
    try {
      return await _repository
          .resendCode(phone: canonicalPhone, intent: intent)
          .timeout(_timeout);
    } on TimeoutException {
      return const PhoneOtpRequestFailure(
        'Resending the verification code took too long. Please try again.',
        code: 'phone_otp_resend_timeout',
      );
    }
  }
}
