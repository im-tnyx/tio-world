...
| Full onboarding parent flow | Scaffolded | `apps/features/onboarding` with Profile, Workout, Nutrition, `apps/shared`, and `apps/app` contracts | Pure step/draft/status contracts, exact mode plans, stable-step reconciliation, a Riverpod-compatible controller, and fixed parent-shell UI are implemented and tested. `/onboarding` still uses the standalone App Mode page; mode migration, owner steps, secure resume, and completion remain pending. |
...
| Profile and Settings entry | Implemented | `apps/features/profile`, `apps/features/settings`, `apps/app`, `apps/core` | Home exposes only the Profile avatar. The minimal Profile launcher owns the Settings entry and preserves the Profile -> Settings -> Profile back stack; profile details remain planned. |
...
| Reusable `TioAvatar` | Implemented | `apps/core` | Shell uses the shared component. Four semantic sizes, circle/rounded shapes, optional image, initials/icon/error fallback, and caller-supplied semantics have widget coverage. Profile upload/storage remains future work. |
