...
| Full onboarding parent flow | Documented | `apps/features/onboarding` with Profile, Workout, Nutrition, `apps/shared`, and `apps/app` contracts | Target is one `/onboarding` parent with fixed top progress, changing child content, fixed bottom actions, stable mode-derived steps, separate draft/confirmed mode, resume, and safe completion. None of this full-flow behavior is implemented yet. |
