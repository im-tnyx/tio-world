...
| Full onboarding parent flow | Scaffolded | `apps/features/onboarding` with Profile, Workout, Nutrition, `apps/shared`, and `apps/app` contracts | Pure step/draft/status contracts, exact mode plans, stable-step reconciliation, a Riverpod-compatible controller, fixed parent-shell UI, and reusable first-child `AppModeStep` are implemented and tested. `/onboarding` still uses the standalone compatibility page; router/completion migration, owner steps, secure resume, and completion remain pending. |
