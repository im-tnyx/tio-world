...
| D-006 | Target, not implemented | `apps/core` will own one reusable `TioAvatar`. | Semantic sizes are `compact`, `small`, `medium`, and `large`; circular is the default and Profile may choose a rounded treatment. |
...
| D-008 | Active direction, not implemented | Phone UI adopts Material 3 Expressive through `apps/core`. | Migrate tokens and shared components incrementally with accessibility, dark-mode, and interaction validation. Do not depend on an assumed separate Flutter M3 Expressive API. Wear remains compact and watch-first. |
