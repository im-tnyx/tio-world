...
| D-006 | Active; implemented | `apps/core` owns one reusable `TioAvatar`. | Semantic sizes are `compact`, `small`, `medium`, and `large`; circular is the default, Profile may choose rounded, and image/fallback/semantics behavior is centralized. |
