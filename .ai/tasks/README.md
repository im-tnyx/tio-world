...
| [TNYX-266 — W6A Library route & capability-gated root](tnyx-266-w6a-library-route.md) | In progress | `apps/features/workout` Library + Workout Home entry, `apps/core` route, `apps/app` router | `/workout/library` with only ready sections (Exercises); Workout Home entry; no Programs/Routines placeholders |
