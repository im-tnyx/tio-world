...
| [TNYX-271 — App startup path organization](tnyx-271-app-startup-organization.md) | Validated | `apps/app` startup/composition root | GitHub #337 / PR #338 merged; startup runtime files grouped under `app/startup/` with behavior preserved |
| [TNYX-201 B1 — Runtime/Auth composition split](tnyx-201-b1-runtime-auth-composition.md) | In progress | `apps/app` composition root | GitHub #359; extract only runtime/API + Auth/session providers while preserving `network_providers.dart` compatibility |
