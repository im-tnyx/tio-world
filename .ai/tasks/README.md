...
| [TNYX-272 — W3A2b Exercises screen & route](tnyx-272-w3a2b-exercises-screen-route.md) | In progress | `apps/features/workout` presentation + `apps/core` route + `apps/app` router | `/workout/exercises` screen over the bundled catalog; no Workout Home/Library entry, no detail/video |
