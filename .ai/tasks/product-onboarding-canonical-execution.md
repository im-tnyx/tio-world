...
**Status:** Ready — next implementation slice is O1 durable App Mode  
...
## O1 — Durable App Mode / active navigation — NEXT

**Tracker:** #11

Why first:
- `user_app_preferences` already exists live;
- current confirmed App Mode remains device-local SharedPreferences;
- second-device/fresh-install recovery is therefore still broken;
- App Mode determines mode-specific onboarding eligibility and post-onboarding guided navigation.

Scope:
- [ ] backend-neutral App Preferences repository/domain contract;
- [ ] Supabase adapter reads/writes `user_app_preferences`;
- [ ] onboarding completion writes `app_mode` + derived ordered `active_tabs` before completion is published;
- [ ] Settings mode change writes the same owner;
- [ ] authenticated bootstrap restores canonical remote preference;
- [ ] valid remote state wins over stale local cache;
- [ ] SharedPreferences becomes cache/pre-auth staging only;
- [ ] no silent Hybrid fallback;
- [ ] mode changes never delete hidden Nutrition/Workout/Body data;
- [ ] fresh install / cleared local data / second-device tests;
- [ ] full Flutter/Dart CI and exact checkpoint recorded.
...
**Current Product Onboarding next slice:** O1 durable App Mode / active-tabs (#11).  
**Current parallel account slice:** A1 contact verification (#8), not blocking O1.  
**After O1 validation:** O2 common Profile owner + section activation.  
