...
| `resolvedFirstDayOfWeek` is a Core parameter defaulting to `MaterialLocalizations.firstDayOfWeekIndex` | Made | the TNYX-72 Settings UI is not required by N2 and no feature-local preference may be created | TNYX-72 |
...
| Meal Diary passes no `resolvedFirstDayOfWeek` at all | Made | week start is one app-wide value; passing anything from Nutrition would make it a second owner. Null falls back to the platform locale, which is the same "automatic" answer the future global resolver produces | TNYX-72 |
